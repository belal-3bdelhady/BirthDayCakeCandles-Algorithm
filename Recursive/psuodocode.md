...
2. max <- 0
3. count <- 0
4. algorithm numOfHighestCandles( candles , total_candles ) //candles is an array of integers 
5. {   
6.     // Base Case
...
T(total_candles) = T(total_candles-1) + 1 , and   T(0) = θ(1).
...
**Time Complexity**
T(total_candles-k)= T(0) = θ(1) , when k = total_candles
Thus  ,	T(total_candles) = θ(1) + θ(total_candles) = θ(total_candles)
